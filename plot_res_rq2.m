function plot_res_rq2(Greens,rmp,U,D,Urmp0,rmp0,dat_ph,dat_az,dat_gps,dat_gov,xP,yP,xA,yA,xG,yG,xO,yO,output,zone,xo,yo)
    ...
    x = [xP;xA;xG;xO];
    y = [yP;yA;yG;yO];
    num_dat = [0 dat_ph dat_az dat_gps*3 dat_gps*3 dat_gov];
    for j = 1:1:length(dat_ph)+length(dat_az)+length(dat_gps)*2+length(dat_gov)
        if j <= length(dat_ph)+length(dat_az) || j > length(dat_ph)+length(dat_az)+length(dat_gps)*2
            if num_dat(j+1)~=0
                xx = x(sum(num_dat(1:j))+1:sum(num_dat(1:j+1)));
                yy = y(sum(num_dat(1:j))+1:sum(num_dat(1:j+1)));
                rr = r(sum(num_dat(1:j))+1:sum(num_dat(1:j+1)));
                sqrt(sum(rr.^2)/length(rr))
                plot_data(xx,yy,rr)
            ...
            if num_dat(j+1)~=0
                ...
                figure, hold on
                quiver(xG(1:end/6),yG(1:end/6),gps_e*s,gps_n*s,0,'r')
                quiver(xG(1:end/6),yG(1:end/6),(gps_e+gps_er)*s,(gps_n+gps_nr)*s,0,'m')
                quiver(xG(1:end/6),yG(1:end/6),0*xG(1:end/6),gps_u*s,0,'b')
                quiver(xG(1:end/6),yG(1:end/6),0*xG(1:end/6),(gps_u+gps_ur)*s,0,'c')
                
                sqrt(sum(gps_er.^2)/length(gps_er))
                sqrt(sum(gps_nr.^2)/length(gps_nr))
                sqrt(sum(gps_ur.^2)/length(gps_ur))
                %{
                figure, hold on
