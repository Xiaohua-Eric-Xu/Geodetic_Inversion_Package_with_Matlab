...
%**   UPDATE LOG:
%**   MATLAB version, Yuri Fialko 11/25/99
%**   Modified by Xiaohua Xu, 11/20/15
%****************************************************************
...
%       PROCESSING STEPS:
	   ...
	   r_v=zeros(length(xi(:)),2);
           ...
           r_v(:,2) = r_k0*(r_m - r_m0 + r_n.*tan(yi).*( r_a2/2.d0 + ...
               ...
               (r_a6/720.d0) ));
         for ii=1:1:length(yi)
           if yi(ii) >= 0 
              r_v(ii,2) = r_v(ii,2) + r_fn(1);
           else
              r_v(ii,2) = r_v(ii,2) + r_fn(2);
           end
         end
	   ...
	   r_vu=zeros(length(xi(:)),2);
        ...
        elseif i_type == 3    % no conversion between UTM and Lat, Lon
           xo=xi*1000;              % convert the km to m
           yo=yi*1000;
           
        elseif i_type == 4     % convert from m to km
            xo=xi/1000;
            yo=yi/1000;
        end
