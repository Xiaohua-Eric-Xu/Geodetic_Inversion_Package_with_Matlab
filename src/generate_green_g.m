function GreenG = generate_green_g(XS,YS,ZS,LL,WW,DIP,STRIKE,xG,yG,tpG,nu,fault_type,gps_type,dat_gps)
    ...
    for j = 1:1:length(XC)
        for m = 1:1:3
            ...
            for k = 1:1:length(dat_gps)
                ...
                if fault_type(m) ~= 0
                    [ux,uy,uz] = calc_okada(1,1,x,y,nu,DIP(j),-ZC(j),LL(j),WW(j),m,STRIKE(j),tpG(l:l-1+dat_gps(k)));
                    if gps_type(1,k) ~= 0
                        G = [G;ux;uy];
                    end
                    if gps_type(2,k) ~= 0
