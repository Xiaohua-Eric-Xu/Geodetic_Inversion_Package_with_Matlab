...
for dipj = 1:1:Ndip
    ...
    if num_gps ~= 0
        GreenGG = generate_green_g(XSS,YSS,ZSS,LLL,WWW,DIPP,STRIKEE,xG1,yG1,tpG1,nu,fault_type,gps_type,dat_gps);
    end
    if num_gov ~= 0
        %[xOO,yOO,dOO,azOO,tpOO,sOO,dat_gov] = proj_gov2trace(patch,xO,yO,dO,azO,tpO,sO,dat_gov,300,800);
        GreenOO = generate_green_o(XSS,YSS,ZSS,LLL,WWW,DIPP,STRIKEE,xOO,yOO,azOO,tpOO,nu,fault_type);
    ...
    %Greenss = [GreenPP;GreenAA;GreenGG;GreenOO];
    Greenss = [GreenPP,GreenPP;GreenAA,GreenAA;[GreenGG,zeros(size(GreenGG));zeros(size(GreenGG)),GreenGG];GreenOO, GreenOO];
    ...
    rmpp = [];
    if RMP == 1
        rmpp = generate_green_ramp(xP,yP,xA,yA,dat_ph,dat_az,dat_gps*2,dat_gov,100); 
    end
    
    %Green_all3 = [Greenss rmpp; SmoothH/mean(max(SmoothH,[],2))*SF/size(SmoothH,1) zeros(size(SmoothH,1),size(rmpp,2))];
    Green_all3 = [Greenss,rmpp; [SmoothH/mean(max(SmoothH,[],2))*SF/size(SmoothH,1),zeros(size(SmoothH));zeros(size(SmoothH)),SmoothH/mean(max(SmoothH,[],2))*SF/size(SmoothH,1)],zeros(size(SmoothH,1)*2,size(rmpp,2))];
    ...
    [U,resnorm,residual,exitflag] = lsqlin([Green_all3;mmt1*dw1;mmt2*dw2],[D_all3;m1*dw1;m2*dw2],[],[],[],[],lb,ub);
    if RMP == 1
        rmp3 = rmp;
        Urmp3 = U(end-size(rmp3,2)+1:end);
        D_all3 = [W.*(D-rmp3*Urmp3); zeros(size(Green_all3,1)-size(D,1),1)];
        rmpp = generate_green_ramp(xP,yP,xA,yA,dat_ph,dat_az,dat_gps*2,dat_gov,1e6);
        Green_all3 = [Greenss,rmpp; [SmoothH/mean(max(SmoothH,[],2))*SF/size(SmoothH,1),zeros(size(SmoothH));zeros(size(SmoothH)),SmoothH/mean(max(SmoothH,[],2))*SF/size(SmoothH,1)],zeros(size(SmoothH,1)*2,size(rmpp,2))];
        for j = 1:1:length(W)
            Green_all3(j,:) = Green_all3(j,:)*W(j);
        end
    
        [U,resnorm,residual,exitflag] = lsqlin([Green_all;mmt1*dw1;mmt2*dw2],[D_all;m1*dw1;m2*dw2],[],[],[],[],lb,ub);
    else
        rmp0 = 0;
        Urmp0 = 0;
    end
