...
% path to where the data and initial model are:
    %earthquake = 'new/Kumamoto';
    earthquake = 'new/Ridgecrest';
...
%% modify the config.inv file under model path to change the parameters
    ...
    MU = cell2mat(inifile(config_inv,'read',{'model','model_params','shear_modulus','d'}));
    ...
    if num_gps ~= 0
        ...
        for j = 1:1:num_gps
            ...
            if gps_h(j) ~= 0
                ...
                sG = [sG;[dat(:,6);dat(:,7)]/10];	% error in East North Up directions (in cm)
...
%%% set lsqlin params and bounds
    ...
    % For Ridgecrest or complex scenario
    bc1 = [-1,-1,-1,-1,-1,1,-1,-1];
    %bc1 = [-1,-1];
    % Regular inversion
    %bc1 = PSC*ones(size(patch));
    
    gd1 = cumsum([0;num_grid]);

    if PSC > 0
        lb(1:sum(fault_type):end-size(rmp,2)) = 0;
    elseif PSC < 0
        % originally ub = 0, lb = -PMAX
        for j = 1:1:length(bc1)
            if bc1(j) > 0
                pu = PMAX; pl = 0;
            elseif bc1(j) < 0
                pu = 0;pl = PMAX;
            ...
            ub(sum(fault_type)*gd1(j)+1:sum(fault_type):sum(fault_type)*(gd1(j+1)-1)+1) = pu*bc1(j);
            lb(sum(fault_type)*gd1(j)+1:sum(fault_type):sum(fault_type)*(gd1(j+1)-1)+1) = pl*bc1(j);
        end
    end
    if PDC > 0
    ...
    options = optimset('LargeScale','on','DiffMaxChange',1e-6,'DiffMinChange',1e-9,'TolCon',1e-9,'TolFun',1e-9,'TolPCG',1e-9,'TolX',1e-9,'MaxIter',1e9,'MaxPCGIter',1e9);
...
%%% plot the residuals and results
    ...
    fprintf(' reduction percentage for each dataset = %f%% %f%% %d%% %d%%, only %d%% to go!\n',pct(1),pct(2),...
        (pct(3)),(pct(4)),400-round(sum(pct)));
...
%}    
    ...
    m = 2/3*log10(mu* sum (sqrt(Us.^2+Ud.^2)/100.*LL.*WW)) - 6.07;
    ...
    r = forwards - D;
    gps_e = D(1:end/3);
    gps_n = D(end/3+1:end/3*2);
    gps_u = D(end/3*2+1:end);
    gps_er = r(1:end/3);
    gps_nr = r(end/3+1:end/3*2);
    gps_ur = r(end/3*2+1:end);
    figure, hold on
...
%%    
    ...
    for i = 1:1:length(D)/3
        fprintf(fid,'%.12f  %.12f  %.12f  %.12f %.12f  %.12f  %.12f  %.12f\n',dat(i,1),dat(i,2),dat(i,3),dat(i,4),dat(i,5),forwards(i)*10,forwards(i+end/3)*10,forwards(i+end/3*2)*10);
    end
    fclose(fid);
    ...
    for k = 1:1:N
        ...
        %    end
        %end
        pct0(k) = 100*(sum(ms0)-sum(ms))/sum(ms0);
        rms0(k) = sqrt(sum(ms)/size(D,1));
    ...
    figure, semilogx(SF0*exp(([1:N]-round(N/2))/2),rms0,'g-x'), grid on, hold on,
    semilogx(SF0,rms0(15),'ro')
    %semilogx(SF0*exp(([1:N]-round(N/2))/2),pct0(:,4),'g-x')
    fid = fopen('sf_rms.txt','w');
    for i = 1:1:N
        fprintf(fid,'%.12f    %.12f\n',SF0*exp((i-round(N/2))/2),rms0(i));
    end
    fclose(fid);
